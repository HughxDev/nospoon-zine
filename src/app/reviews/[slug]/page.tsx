...
// HVML
// interface HVMLReview {}

const dataDirectory = path.join( process.cwd(), 'data' );

export async function getReview( slug: string ) {
  // const file = await fs.readFile( `${dataDirectory}/${slug}.hvml`, 'utf-8' );
  const path = `${dataDirectory}/${slug}.hvml`;

  console.log( 'path', path );
  
  const hvml = new HVML(
    `${dataDirectory}/${slug}.hvml`,
    {
      schemaPath: 'node_modules/hvml/rng/hvml.rng',
    },
  );
  let parsed;

  // try {
  //   parsed = await hvml.ready;
...
export default async function Review( { params }: NextPageProps ) {
  // const file = await getReview( params.slug );
                    ...
                    { videoMentions.map( ( { expandedTitle, lang, published } ) => {
                      const $videoId = (
                        <hgroup>
                          ...
                          >
                            { expandedTitle }
                          </cite> (<time dateTime={ published }>{ published.split( '-' )[0] }</time>)</h5>
                        </hgroup>
                      );
